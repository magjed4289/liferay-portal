...
1. **Carried Forward From Past Analyses** — one row per subtask/cluster migrated in **Match Adjacent-Task Subtasks** or **Sync Claims from Past Analyses**, not per individual test: subtask badge, test count, a **Component(s)** column (same resolution as **New Regressions** — [`references/testray-testflow.md`](references/testray-testflow.md#resolve-a-subtasks-components), across every member case of every subtask badge in the row, since one ticket can span several subtasks), ticket(s), assignee, merge status, verdict (from **Check Whether a Carried-Forward Fix Already Landed**), and an **Acceptance** column (from **Cross-Check Against the Acceptance Routine**, same values as the New Regressions table). Keep every cell terse — a short label, not a paragraph:
