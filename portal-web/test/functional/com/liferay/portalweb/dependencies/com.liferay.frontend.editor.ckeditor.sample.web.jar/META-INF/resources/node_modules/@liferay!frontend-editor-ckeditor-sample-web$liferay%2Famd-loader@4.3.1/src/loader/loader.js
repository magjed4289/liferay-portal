Liferay.Loader.define('@liferay!frontend-editor-ckeditor-sample-web$liferay/amd-loader@4.3.1/src/loader/loader', ['module', 'exports', 'require'], function (module, exports, require) {
   ...
   */
    ...
    */
			constructor(config = null, document = null) {
				...
				this._scriptLoader = new ScriptLoader(document || window.document, this._config, this._log);
    ...
    */
			define(...args) {
				...
				if (module && module.defined) {
					this._log.warn(`Module '${name}' is being redefined. Only the first`, 'definition will be used');
			...
			require(...args) {
				...
				if (args.length == 1) {
					...
					if (!successGiven) {
						...
						if (typeof penultimateArg === 'function' || penultimateArg == null) {
							failureGiven = true;
						}
				...
				this._dependencyResolver.resolve(moduleNames).then(resolution => {

					// Show extra information when explainResolutions is active

					this._log.resolution('Require call', requireCallId, 'resolved modules', moduleNames, 'to', resolution);
					...
					if (resolution.errors && resolution.errors.length > 1) {
						throw Object.assign(new Error('The server generated some errors while ' + 'resolving modules'), { resolutionErrors: resolution.errors });
					}
					...
					unregisteredModuleNames = this._getUnregisteredModuleNames(resolvedModuleNames);
					...
					unregisteredModuleNames.forEach(moduleName => {
						...
						let moduleFlags = resolution.moduleFlags ? resolution.moduleFlags[moduleName] : {};
					...
					rejectTimeout = this._setRejectTimeout(moduleNames, resolution, (...args) => {
					...
					this._log.resolution('Fetching', unregisteredModuleNames, 'from require call', requireCallId);
					...
					this._setModuleImplementations(requireCallId, resolvedModuleNames);

					// Now get all needed modules implementations

					const implementations = this._getModuleImplementations(moduleNames);
    ...
    */
			_interceptHandler(handler, type, requireCallId) {
				return (...args) => {
					this._log.resolution('Invoking', type, 'handler for', 'require call', requireCallId);
    ...
    */
			_logServerMessages(moduleNames, resolution) {
				if (resolution.errors && resolution.errors.length > 0) {
					this._log.error('Errors returned from server for require(', moduleNames, '):', resolution.errors);
				}

				if (resolution.warnings && resolution.warnings.length > 0) {
					this._log.warn('Warnings returned from server for require(', moduleNames, '):', resolution.warnings);
				}
			}
    ...
    */
			_setRejectTimeout(modules, resolution, reject) {
				...
				return setTimeout(() => {
					...
					const error = Object.assign(new Error('Load timeout for modules: ' + modules), {
						missingModules,
						modules,
						resolution
					});
    ...
    */
			_throwOnLegacyProtocolResolutionErrors(resolution) {
				const resolutionErrors = resolution.resolvedModules.filter(dep => dep.indexOf(':ERROR:') === 0).map(dep => dep.substr(7));

				if (resolutionErrors.length > 0) {
					throw Object.assign(new Error('The following problems where detected while ' + 'resolving modules:\n' + resolutionErrors.map(line => `    · ${line}`).join('\n')), { resolutionErrors });
				}
			}
    ...
    */
			_waitForModuleDefinitions(moduleNames) {
				...
				return Promise.all(config.getModules(moduleNames).map(module => module.define));
			}
    ...
    */
			_waitForModuleImplementations(moduleNames) {
				...
				return Promise.all(config.getModules(moduleNames).map(module => module.implement));
			}
    ...
    */
			_setModuleImplementations(requireCallId, moduleNames) {
				...
				config.getModules(moduleNames).forEach(module => {
					...
					this._log.resolution('Implementing', module.name, 'from require call', requireCallId);
					...
					try {
						...
						const dependencyImplementations = module.dependencies.map(dependency => {
							if (dependency === 'exports') {
								...
								const dependencyModule = config.getDependency(module.name, dependency);

								if (!dependencyModule) {
									throw new Error(`Unsatisfied dependency: ${dependency} ` + `found in module ${module.name}`);
								}

								if (!dependencyModule.implementation && !dependencyModule.implemented) {
									throw new Error('Module "' + dependencyModule.name + '" has not been loaded yet for context: ' + module.name);
								}
			...
			_createLocalRequire(module) {
				...
				const localRequire = (moduleName, ...rest) => {
					if (rest.length > 0) {
						...
						const dependencyModule = config.getDependency(module.name, moduleName);
    ...
    */
			_getModuleImplementations(moduleNames) {
				...
				return config.getModules(moduleNames).map(module => module.implementation);
			}
		}
